...
class DDPG(object):
    def __init__(self, actor, critic, memory, observation_shape, action_shape, param_noise=None, action_noise=None,
        ...
        critic_l2_reg=0., actor_lr=1e-4, critic_lr=1e-3, clip_norm=None, reward_scale=1., epsilon=1e-3, latent_shape=(258,),
        ...
        self.actor_tf = actor(normalized_obs0, self.additional0)
        self.normalized_critic_tf = critic(normalized_obs0, self.additional0, self.actions)
        self.critic_tf = denormalize(tf.clip_by_value(self.normalized_critic_tf, self.return_range[0], self.return_range[1]), self.ret_rms)
        self.normalized_critic_with_actor_tf = critic(normalized_obs0, self.additional0, self.actor_tf, reuse=True)
        self.critic_with_actor_tf = denormalize(tf.clip_by_value(self.normalized_critic_with_actor_tf, self.return_range[0], self.return_range[1]), self.ret_rms)
        Q_obs1 = denormalize(target_critic(normalized_obs1, self.additional1, target_actor(normalized_obs1, self.additional1)), self.ret_rms)
        self.critic_target = self.rewards + (1. - self.terminals1) * gamma * Q_obs1

        # Set up parts.
        if self.param_noise is not None:
            self.setup_param_noise(normalized_obs0, self.additional0)
    ...
    def setup_param_noise(self, normalized_obs0, additional0):
        assert self.param_noise is not None

        # Configure perturbed actor.
        param_noise_actor = copy(self.actor)
        param_noise_actor.name = 'param_noise_actor'
        self.perturbed_actor_tf = param_noise_actor(normalized_obs0, additional0)
        ...
        adaptive_actor_tf = adaptive_param_noise_actor(normalized_obs0, additional0)
